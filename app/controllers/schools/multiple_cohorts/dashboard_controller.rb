...
class Schools::MultipleCohorts::DashboardController < Schools::BaseController
  ...
  def show
    @partnership = @school.partnerships.active.find_by(cohort: Cohort.find_by(start_year: params[:cohort]))
...
private
  ...
  def set_school_cohorts
    ...
    @cohort_list = [Cohort.find_by(start_year: params[:cohort])]
